...
check_type3(_LogFun, _Key, _Ckey, tii,   Cvalue, _Dvalue) ->
	case ct_validate(tii, Cvalue) of
		true  -> ok;
		_     -> invalid
	end;

check_type3(_LogFun, _Key, _Ckey, tiii,   Cvalue, _Dvalue) ->
	case ct_validate(tiii, Cvalue) of
		true  -> ok;
		_     -> invalid
	end;
...
check1_one_default(LogFun, Default) when is_tuple(Default) ->
	try
		...
		%% TODO is this necessary since we have the line above?
...
%% tii -> {int(), int()}
%%
check3_one_default(LogFun, {Key, Level, Type, Value}) when Type==tii ->
	case ct_validate(tii, Value) of
		true  -> {Key, Level, Type, Value};
		false -> log(LogFun, debug, "config: expecting 'tii' type for Key: ", [Key]), {}
	end;

check3_one_default(LogFun, {Key, Level, Type, Value}) when Type==taa ->
	case ct_validate(taa, Value) of
		true  -> {Key, Level, Type, Value};
		false -> log(LogFun, debug, "config: expecting 'taa' type for Key: ", [Key]), {}
	end;

check3_one_default(LogFun, {Key, Level, Type, Value}) when Type==tai ->
	case ct_validate(tai, Value) of
		true  -> {Key, Level, Type, Value};
		false -> log(LogFun, debug, "config: expecting 'tai' type for Key: ", [Key]), {}
	end;

check3_one_default(LogFun, {Key, Level, Type, Value}) when Type==taii ->
	case ct_validate(taii, Value) of
		true  -> {Key, Level, Type, Value};
		false -> log(LogFun, debug, "config: expecting 'taii' type for Key: ", [Key]), {}
	end;

check3_one_default(LogFun, {Key, Level, Type, Value}) when Type==tiii ->
	case ct_validate(tiii, Value) of
		true  -> {Key, Level, Type, Value};
		false -> log(LogFun, debug, "config: expecting 'tiii' type for Key: ", [Key]), {}
	end;
...
check3_one_default(LogFun, {Key, _Level, _Type, _Value}) ->
	log(LogFun, debug, "config: invalid default for Key: ", [Key]).
...
%%		{int(), int()}
%%
ct_validate(tii, Value) when is_tuple(Value) ->
	try
		{V1, V2} = Value,
...
%% @doc Validate Compound Type 'tiii'
%%		{int(), int(), int()}
%%
ct_validate(tiii, Value) when is_tuple(Value) ->
	try
		{V1, V2, V3} = Value,
		is_integer(V1) and is_integer(V2) and is_integer(V3)
	catch
		_:_ -> false
	end;
